...
_mainScript_() {
  _executeScript_ "$(_findBaseDir_)/system_checker.sh"
  _executeScript_ "$(_findBaseDir_)/traefik.sh"
  _executeScript_ "$(_findBaseDir_)/portainer.sh"
  _executeScript_ "$(_findBaseDir_)/supabase.sh"
  _executeScript_ "$(_findBaseDir_)/databases.sh"
  _executeScript_ "$(_findBaseDir_)/monorepo.sh"
  _executeScript_ "$(_findBaseDir_)/dashboards.sh"

}

# Function: Run a script and check its execution status
_executeScript_() {
  local script_path="$1"
